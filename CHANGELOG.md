...
- You can now bind a physical remote to a motor from the Home Assistant UI by pressing the remote's button. Open a motor's **Configure** screen and choose **Bind a remote**, then press the remote button twice — Home Assistant shows the captured remote for confirmation before saving it. A button that belongs to a motor, or a remote already bound to another motor, is rejected with a clear message, and you can change or remove the binding later from the same screen. The binding is stored with the motor so it survives restarts and immediately enables remote-move position tracking and the remote-button event entity.
